...
def main():
    ...
    if st.session_state.jd_texts:
        ...
        if jd_to_delete:
            for jd_name in jd_to_delete:
                st.session_state.jd_texts.pop(jd_name, None)
    ...
    if st.session_state.resume_texts:
        ...
        if resume_to_delete:
            for resume_name in resume_to_delete:
                st.session_state.resume_texts.pop(resume_name, None)
    ...
    if not df_all.empty:
        ...
        st.subheader("Candidate Verdict Summary")
    ...
    if not df_all.empty:
        ...
        jd_names_unique = df_all["jd_name"].unique()
        for jd in jd_names_unique:
            st.markdown(f"### {jd}")
